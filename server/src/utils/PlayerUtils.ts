...
export class PlayerUtils {
  ...
  getRandomIndex(totalPlayers: number) : number {
    let randomPlayer = Math.random() * Math.floor(totalPlayers);
    return Math.floor(randomPlayer);
  }

  determineWinners(players: MapSchema<CPlayer>) : ArraySchema<CPlayer> {
    let winningPlayers: string[] = [];
    let winningHand: number = -1;
    for(let key in players) {
      console.log(`Player id ${players[key].sessionId} and hand ${players[key].hand}`);
      if(winningHand === -1) {
    ...
    let winners: ArraySchema<CPlayer> = new ArraySchema<CPlayer>();
